        ...
        if not os.path.exists(pdf_file_path):
            ...
            if not pdf_files:
                ...
                if customer_id in recommendations_storage:
                    recommendations_data = recommendations_storage[customer_id]
                    generate_pdf_report(customer_id, recommendations_data)
                    ...
                    if not pdf_files:
                        raise HTTPException(
                            status_code=404,
                            detail=f"PDF report not found for customer {customer_id}. Please try again in a few moments."
                        )
                else:
                    raise HTTPException(
                        status_code=404,
